  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e) => {
      if (e.touches.length === 1) {
        ...
        // Enhanced mobile scrolling sensitivity
        let scrollDelta = 0;
        if (Math.abs(deltaX) > Math.abs(deltaY)) {
          // Horizontal swipe - primary navigation
          scrollDelta = deltaX * 0.2;
        } else {
          // Vertical swipe - secondary navigation  
          scrollDelta = deltaY * 0.15;
        }
  ...
  const NavigationItem = ({ icon, label, subItems, itemKey, index }) => {
    ...
    for (let i = 0; i < index; i++) {
      ...
      if (expandedSection === prevItemKey && sidebarOpen) {
        ...
        topPosition += prevSubItems.length * 30 + 20; // Add space for expanded sub-items
      }
    }
      ...
      <div style={{
        ...
        >
          ...
          {sidebarOpen && (
            <span style={{
              position: 'absolute',
              left: '100px', // To the right of the centered icon area
              top: '50%',
        ...
        {sidebarOpen && (
          <div style={{
            marginLeft: '100px', // Aligned with label position
            maxHeight: isExpanded ? `${(subItems?.length || 0) * 30 + 20}px` : '0px',
            overflow: 'hidden',
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        {/* YC Logo - Fixed pixel position, never moves */}
        <div style={{ 
          position: 'absolute',
          bottom: '20px',
          left: '40px',
