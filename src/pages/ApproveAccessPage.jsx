...
/**
 * ApproveAccessPage - Handles client access approval
 * Redirects to Firebase function with token
 */
function ApproveAccessPage() {
  ...
  useEffect(() => {
    ...
    window.location.href = `https://us-central1-yellowcircle-app.cloudfunctions.net/approveAccess?token=${token}`;
  }, [searchParams]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          width: '80px',
          height: '80px',
          backgroundColor: status === 'error' ? '#ef4444' : '#fbbf24',
          borderRadius: '50%',
          margin: '0 auto 20px',
          display: 'flex',
          alignItems: 'center',
        ...
        <h1 style={{ color: '#1f2937', fontSize: '24px', margin: '0 0 10px' }}>
          {status === 'processing' ? 'Approving Access...' : 'Error'}
        </h1>
